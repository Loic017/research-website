...
export function PortfolioEntry({ portfolio }: { portfolio: Portfolio }) {
  return (
    <div className="flex flex-col sm:flex-row gap-6">
      {portfolio.imageUrl && (
        <div className="w-1/2 min-w-[240px] relative">
          <Image
      ...
      <div className="flex flex-col flex-1">
        ...
        {portfolio.technologies && (
          <div className="flex gap-2 mb-4 flex-wrap">
            {portfolio.technologies.map((tech, index) => (
              <span
                key={index}
                className="text-xs text-muted px-2 py-1 bg-surface-2 rounded-full"
              >
                {tech}
              </span>
            ))}
        ...
        <p className="text-sm text-muted mb-4 mt-4 italic">
